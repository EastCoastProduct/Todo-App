		...
		return <div className='profile'>
			...
			<div className='paddingTop'><h2>{ this.state.firstName }&nbsp;{ this.state.lastName }</h2></div>
                    ...
                    {this.state.description ? (<div>
										...
										<div className='description'>{ this.state.description }</div></div>) : (<div></div>)}


                    {(!this.state.isAdmin && this.state.modules != '') ? (
                        <div><div className='clr'></div>
												<div className=' paddingTop lightFont'>Finished modules</div><div>{ _singleItems }</div></div>) :
                    (<div></div>)}
